		...
		public:
			...
			static HashTable<int32_t, int32_t> * run(const Graph & g, uint32_t src_id) {
                ...
                for ( auto it = m.cbegin(); it != m.cend(); ++it ) {
					dist[it->second->v.id] = INT_MAX; // set inital distance to each vertex as INT_MAX
					(*previous)[it->second->v.id] =  UNDEFINED; // clear path to UNDEFINED
					visited[it->second->v.id] = false; // all vertices are not visited
                }
